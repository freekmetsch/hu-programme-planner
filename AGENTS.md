...
- GitHub Pages serves `/` from `codex/publish-programme-planner`. Read the Pages API for the exact live URL and build status.
- The site is static and has no authentication or health endpoint. After deployment, check the root page, all three views, course filters, exact-code equivalence, comparison, and source links in a logged-out browser.
